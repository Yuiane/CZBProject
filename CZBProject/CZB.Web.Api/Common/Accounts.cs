...
namespace CZB.Web.Api.Common
{
    ...
    public partial class Accounts
    {
        ...
        /// <param name="phone"></param>
        /// <returns></returns>
        public UserLoginReturn UserLoginThird(string thirdOpenId)
        {
            UserLoginReturn returnModel = null;
            var model = new BLL.FX_Agent().GetModelByThirdOpenId(thirdOpenId);
            if (model != null)
            {
                returnModel = new UserLoginReturn();
                //用户基本信息
                int level = model.AgentLevel.ToInt32();
                int agentId = model.AgentId.ToInt32();
                returnModel.userInfo = new UserInfo()
